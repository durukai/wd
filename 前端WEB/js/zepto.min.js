...
var jQuery = Zepto;
;$(document).on('ajaxSuccess',function(event,xhr,settings){if(window['WXD_HELP_DATA']){try{var data=$.parseJSON(xhr.responseText);switch(data.status){case'nologin':if(WXD_HELP_DATA[11]){if(WXD_HELP_DATA[2][0]=='weixin'){window.location.href=WXD_HELP_DATA[4].weixin+'/usercenter/login/?'+wxid+'&url='+encodeURIComponent(window.location.href);}else{window.location.href=WXD_HELP_DATA[4].member_touch+'/login/?url='+encodeURIComponent(window.location.href);};}else{window.location.href=WXD_HELP_DATA[4].member+'/login/?url='+encodeURIComponent(window.location.href);};break;case'goto':if(data.Message){alert(data.Message);};window.location.href=data.url;break;};}catch(e){};};});